...
var Summary = ({property}) => {
  ...
  return (
    <div className="summary">
      <div className='line1'>
        <span id="sumary_price">$ {property.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
        {vDivider}
        <span id="summary_bed"> {property.bd} bd</span>
        {vDivider}
        <span id="summary_bath">{property.ba} ba</span>
        {vDivider}
        <span id="summary_bath">{property.sqft.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} sqft </span>
      </div>
      ...
      <div className="line3">
        <div>
          ...
          {vDivider}
          <span id="summary_status">Zestimage: </span>
          <span>$ {property.zestimate.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span>
        </div>
      </div>
      <div className="line4">
        <span id="estPayment">Est. payment: ${property.estPayment}/mo</span>
