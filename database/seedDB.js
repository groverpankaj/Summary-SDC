...
var saveNData = (n) => { //helper function for seeding n data into database
  ...
  var promiseAll = Promise.all( // use promise 
    temp.map((ele,i) => {
      ...
      let property = new House({ // generate random information for each document (house property)
        id: i+1,
      ...
      return property.save();; // save each property in collection and return promise generated by save method
    })
  );

  promiseAll
    .then((log) => { // check if every property document is saved in collection 'summaries'
